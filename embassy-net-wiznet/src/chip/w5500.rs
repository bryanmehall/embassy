...
impl super::SealedChip for W5500 {
    ...
    // This W5500-EVB-Pico2's MAC filter (bit 7) drops unicast-to-SHAR even when
    // SHAR matches the advertised MAC, so DHCP OFFER/ACK (unicast) never arrives
    // (multicast/broadcast pass). Disable it -> promiscuous MACRAW mode.
